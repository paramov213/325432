...
io.on('connection', (socket) => {
    ...
    socket.on('call_user', (data) => {
        if (users[data.to]) { io.to(users[data.to]).emit('incoming_call', { from: data.from }); }
    });
    socket.on('accept_call', (data) => {
        if (users[data.to]) { io.to(users[data.to]).emit('call_accepted', { from: data.from }); }
    });
    socket.on('reject_call', (data) => {
        if (users[data.to]) { io.to(users[data.to]).emit('call_rejected'); }
    });
